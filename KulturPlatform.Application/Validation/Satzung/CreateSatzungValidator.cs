...
{
    public class CreateSatzungValidator : AbstractValidator<CreateSatzungCommand>
    {
        public CreateSatzungValidator()
        {
            // Title
            ...
            RuleForEach(x => x.Memberships).ChildRules(m =>
            {
                m.RuleFor(mem => mem.Type)
                    ...
                    .Must(sc => !string.IsNullOrWhiteSpace(sc.BodyTurkish) || !string.IsNullOrWhiteSpace(sc.BodyGerman))
                    .WithMessage("Membership Type Body boş olamaz.");

                m.RuleFor(mem => mem.DescriptionTurkish)
                    .NotNull().WithMessage("Membership Description (TR) boş olamaz.")
                    .Must(sc => !string.IsNullOrWhiteSpace(sc.BodyTurkish))
                    .WithMessage("Membership Description (TR) Body boş olamaz.");

                m.RuleFor(mem => mem.DescriptionGerman)
                    .NotNull().WithMessage("Membership Description (DE) boş olamaz.")
                    .Must(sc => !string.IsNullOrWhiteSpace(sc.BodyGerman))
                    .WithMessage("Membership Description (DE) Body boş olamaz.");
            });
            ...
            RuleFor(x => x.SeatTurkish)
                ...
                .Must(sc => !string.IsNullOrWhiteSpace(sc.BodyTurkish))
                .WithMessage("Seat (TR) Body boş olamaz."); ;

            RuleFor(x => x.SeatGerman)
                .NotNull().WithMessage("Seat (DE) boş olamaz.")
                .Must(sc => !string.IsNullOrWhiteSpace(sc.BodyGerman))
                .WithMessage("Seat (DE) Body boş olamaz.");
